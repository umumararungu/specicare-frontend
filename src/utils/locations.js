export const DISTRICT_SECTORS = {
  Gasabo: [
    'Bumbogo','Gatsata','Gisozi','Gikondo','Kacyiru','Kimironko','Nyarutarama','Remera','Rusororo'
  ],
  Kicukiro: ['Kagarama','Gahanga','Nyarugunga','Nyarugenge','Kicukiro','Niboye'
  ],
  Nyarugenge: ['Nyamirambo','Kibagabaga','Nyakabanda','Muhima','Gitega'
  ],
  Bugesera: ['Gashora','Juru','Kamabuye','Mareba','Mayange','Musenyi','Mwogo','Ngeruka','Ntarama','Nyamata','Nyarugenge','Rilima','Ruhuha','Rweru','Shyara'
  ],
  Gatsibo: ['Gasange','Gatsibo','Gitoki','Kabarore','Kageyo','Kiramuruzi','Kiziguro','Muhura','Murambi','Ngarama','Nyagihanga','Remera','Rugarama','Rwimbogo'
  ],
  Kayonza: ['Gahini','Kabare','Kabarondo','Mukarange','Murama','Murundi','Mwiri','Ndego','Nyamirama','Rukara','Ruramira','Rwinkwavu'
  ],
  Kirehe: ['Gahara','Gatore','Kigarama','Kigina','Kirehe','Mahama','Mpanga','Musaza','Mushikiri','Nasho','Nyamugari','Nyarubuye'
  ],
  Ngoma: ['Gashanda','Jarama','Karembo','Kazo','Kibungo','Mugesera','Murama','Mutenderi','Remera','Rukira','Rukumberi','Rurenge','Sake','Zaza'
  ],
  Nyagatare: ['Gatunda','Karama','Karangazi','Katabagemu','Kiyombe','Matimba','Mimuli','Mukama','Musheli','Nyagatare','Rukomo','Rwempasha','Rwimiyaga','Tabagwe'
  ],
  Rwamagana: ['Fumbwe','Gahengeri','Gishari','Karenge','Kigabiro','Muhazi','Munyaga','Munyiginya','Musha','Muyumbu','Mwulire','Nyakariro','Nzige','Rubona'
  ],
  Burera: ['Bungwe','Butaro','Cyanika','Cyeru','Gahunga','Gatebe','Gitovu','Kagogo','Kinoni','Kinyababa','Kivuye','Nemba','Rugarama','Rugendabari','Ruhunde','Rusarabuye','Rwerere'
  ],
  Gakenke: ['Busengo','Coko','Cyabingo','Gakenke','Gashenyi','Mugunga','Janja','Kamubuga','Karambo','Kivuruga','Mataba','Minazi','Muhondo','Muyongwe','Muzo','Nemba','Ruli','Rusasa','Rushashi'
  ],
  Gicumbi: ['Bukure','Bwisige','Byumba','Cyumba','Giti','Kaniga','Manyagiro','Miyove','Kageyo','Mukarange','Muko','Mutete','Nyamiyaga','Nyankenke II','Rubaya','Rukomo','Rushaki','Rutare','Ruvune','Rwamiko','Shangasha'
  ],
  Musanze: ['Busogo','Cyuve','Gacaca','Gashaki','Gataraga','Kimonyi','Kinigi','Muhoza','Muko','Musanze','Nkotsi','Nyange','Remera','Rwaza','Shingiro'
  ],
  Rulindo: [
    'Base', 'Burega', 'Bushoki', 'Buyoga', 'Cyinzuzi', 'Cyungo', 'Kinihira', 'Kisaro', 'Masoro', 'Mbogo', 'Murambi', 'Ngoma', 'Ntarabana', 'Rukozo', 'Rusiga', 'Shyorongi', 'Tumba'
  ],
  Gisagara: [
    'Gikonko', 'Gishubi', 'Kansi', 'Kibilizi', 'Kigembe', 'Mamba', 'Muganza', 'Mugombwa', 'Mukindo', 'Musha', 'Ndora', 'Nyanza', 'na Save'
  ],
  Huye: [
    'Gashanda', 'Gikonko', 'Huye', 'Kabwayi', 'Kinazi', 'Maraba', 'na Ngoma'
  ],
  Kamonyi: [
    'Gacurabwenge', 'Karama', 'Kayenzi', 'Kayumbu', 'Kigoma', 'Mbuye', 'Musambira', 'Ngamba', 'Nyamabuye', 'Rugendabari', 'Rukoma'
  ],
  Muhanga: [
    'Cyeza', 'Kabacuzi', 'Kabahire', 'Kigoma', 'Mbuye', 'Mushishiro', 'Nyamabuye', 'Rugendabari', 'Rukoma'
  ],
  Nyamagabe: [
    'Busanze', 'Cyahinda', 'Kaduha', 'Kamegeli', 'Kibirizi', 'Mugano', 'Musange', 'Ndora', 'Ruhombo', 'Ruramba', 'Save'
  ],
  Nyanza: [
    'Busasamana', 'Cyabakamyi', 'Kigoma', 'Mugombwa', 'Mukingo', 'Nyamabuye', 'Ruhango', 'Rukoma'
  ],
  Nyaruguru: [
    'Cyahinda', 'Kibeho', 'Muganza', 'Nyabimata', 'Nyakarenzo', 'Ruhombo', 'Ruramba'
  ],
  Ruhango: [
    'Bweramana', 'Kinazi', 'Mbuye', 'Mushishiro', 'Nyamabuye', 'Ruhango', 'Rukoma'
  ],
  Karongi: [
    'Bwishyura', 'Gitesi', 'Gisovu', 'Kabuga', 'Kagano', 'Kanjongo', 'Murambi', 'Mutuntu', 'Rubengera', 'Rugabano', 'Ruhango'
  ],
  Nyabihu: [
    'Bigogwe', 'Busengo', 'Cyabingo', 'Gihango', 'Janja', 'Karago', 'Kintobo', 'Muhondo', 'Nyabihu', 'Rugerero', 'Shyira'
  ],
  Ngororero: [
    'Bwira', 'Cyabingo', 'Kageyo', 'Kigeyo', 'Mubuga', 'Ngororero', 'Rurembo'
  ],
  Nyamasheke: [
    'Bushekeri', 'Cyato', 'Cyangugu', 'Kagano', 'Karambi', 'Katabagemu', 'Nyamasheke', 'Ramba', 'Shangi'
  ],
  Rubavu: [
    'Bugarama', 'Gisenyi', 'Kanama', 'Mudende', 'Nyundo', 'Rubavu', 'Rutsiro'
  ],
  Rusizi: [
    'Bugarama', 'Butare', 'Cyahinda', 'Kamembe', 'Muganza', 'Mururu', 'Nkanka', 'Nyakabuye', 'Rusizi'
  ],
  Rutsiro: [
    'Gihango', 'Kagano', 'Karambo', 'Nyabirasi', 'Rutsiro', 'Rwariro'
  ]
};

export const DISTRICTS = Object.keys(DISTRICT_SECTORS);

export function getSectors(district) {
  if (!district) return [];
  return DISTRICT_SECTORS[district] || [];
}

export default {
  DISTRICT_SECTORS,
  DISTRICTS,
  getSectors
};
